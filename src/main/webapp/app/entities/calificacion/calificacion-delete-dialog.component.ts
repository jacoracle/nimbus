...
export class CalificacionDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ calificacion }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(CalificacionDeleteDialogComponent as Component, { size: 'lg', backdrop: 'static' });
        this.ngbModalRef.componentInstance.calificacion = calificacion;
        this.ngbModalRef.result.then(
          result => {
            this.router.navigate(['/calificacion', { outlets: { popup: null } }]);
